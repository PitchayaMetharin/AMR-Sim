  ...
  sim::EventManager &)
{
  ...
  // The hardware plugin declares simulation parameters (including its
  // position gain) during initSim().  Pass the SDF-provided parameter files
  // when this node is constructed so those declarations receive their YAML
  // overrides.  Parsing the files into the context later is sufficient for
  // the subsequently-created controller manager, but is too late for this
